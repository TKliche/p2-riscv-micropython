...
{
  int32_t 	bufptr, f, b, ch;
  
  bufptr = (int32_t)(self->screen_buffer);
  bufptr = bufptr + (((self->cury * VGATEXT_COLS) + self->curx) * 8);
  f = ((int32_t *)bufptr)[0];
  b = ((int32_t *)(bufptr + 4))[0];
  ch = f & 0xff;
  f &= 0xffffff00;
  if (f == b) {
      f = self->fgcolor;
  ...
  ((int32_t *)bufptr)[0] = b | ch;
  ((int32_t *)(bufptr + 4))[0] = f;
}

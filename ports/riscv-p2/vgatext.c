// automatically generated by spin2cpp v3.9.28-beta-9a18aa43 on Sat Jul 13 18:00:05 2019
...
static  int32_t vgatext_bound(int32_t low, int32_t x, int32_t high);
...
static int32_t vgatext_bound(int32_t low, int32_t x, int32_t high)
{
  return (Min__((Max__(low, x)), high));
}

void vgatext_set_position(vgatext *self, int32_t x, int32_t y)
{
  self->curx = vgatext_bound(0, x, VGATEXT_COLS - 1);
  self->cury = vgatext_bound(0, y, VGATEXT_ROWS - 1);
}
...
int32_t vgatext_tx(vgatext *self, int32_t c)
{
  ...
  if (self->state == VGATEXT_STATE_NORMAL) {
    if (c == 8) {
      // backspace
      vgatext_set_position(self, self->curx - 1, self->cury);
    } else {
      if (c == 10) {
        // line feed
        if (self->cury == self->scroll_last) {
        ...
        if (c == 13) {
          ...
          if (c == 20) {
            // tab
            vgatext_set_position(self, (self->curx + 8) & ((7 << 24) >> 24), self->cury);
          } else {
            if (c == 27) {
...
static void vgatext_handle_esc(vgatext *self, int32_t c)
{
  ...
  if (c == '[') {
    ...
    if (c == 'D') {
      ...
      if (c == 'M') {
        if (self->cury == self->scroll_first) {
...
static void vgatext_handle_csi(vgatext *self, int32_t c)
{
                    ...
                    if (c == 'K') {
                      ...
                      if (c == 'm') {
                        ...
                        if (c == 'r') {
                          ...
                          self->scroll_first = vgatext_bound(0, self->scroll_first, VGATEXT_ROWS - 1);
                          self->scroll_last = vgatext_bound(self->scroll_first, self->scroll_last, VGATEXT_ROWS - 1);
                          self->curx = self->cury = 0;
                        } else {
                          if (c == 's') {
  ...
  // sanity check
  vgatext_set_position(self, self->curx, self->cury);
  self->state = VGATEXT_STATE_NORMAL;
}
